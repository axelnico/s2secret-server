...
impl EmergencyContact {
    ...
    pub async fn add_emergency_contact_for_user(email: &String, description: Option<&String>, server_share: &Vec<u8> ,user_id: &Uuid, database: &PgPool) -> Self {
        let new_emergency_contact_id = Uuid::new_v4();
        sqlx::query!("INSERT INTO emergency_contact(id_emergency_contact, email, description, server_share, user_id) VALUES ($1,$2,$3,$4,$5)",new_emergency_contact_id, email, description, server_share, user_id ).execute(database).await.unwrap();
