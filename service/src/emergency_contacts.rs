...
impl EmergencyContact {
    pub async fn emergency_contacts_of_secret(secret_id: &Uuid, database: &PgPool) -> Vec<Self> {
        sqlx::query_as!(Self, "SELECT ec.id_emergency_contact,ec.email,ec.description from emergency_contact ec \
                                inner join emergency_contact_secret_access eca on ec.id_emergency_contact = eca.id_emergency_contact where eca.id_secret = $1",secret_id)
            .fetch_all(database).await.unwrap()
    }
    pub async fn add_emergency_contact_to_secret(secret_id: &Uuid, emergency_contact_id: &Uuid, server_ticket: &String, server_v: &String, server_a: &String, database: &PgPool) -> Option<Uuid> {
        let emergency_contact_uuid = sqlx::query_scalar!("SELECT id_emergency_contact from emergency_contact where id_emergency_contact=$1",emergency_contact_id).fetch_optional(database).await.unwrap();
    ...
    pub async fn emergency_contacts(database: &PgPool) -> Vec<Self> {
        sqlx::query_as!(Self, "SELECT id_emergency_contact,email,description from emergency_contact").fetch_all(database).await.unwrap()
    }
}